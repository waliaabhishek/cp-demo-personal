...
async def invoke_urls(input_uri: list, **kwargs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENT_THREADS) as executor:
        future_to_url = (executor.submit(invoke_call, uri, user="superUser", password="superUser", kwargs=kwargs) for uri in input_uri)
    ...
    for item in generate_metrics_object_result:
        ...
        if JMX_METRICS_ATTR_TASK_ID in item.keys():
            key_string = JMX_METRICS_BEAN_NAME + ":" \
                ...
                + JMX_METRICS_ATTR_TASK_ID + "=" + str(item[JMX_METRICS_ATTR_TASK_ID])
...
if __name__ == "__main__":
    ...
    connectors_status = [CORE_ENDPOINTS[1].replace("{name}", k) for k in itertools.chain.from_iterable(connectors_list)]
    connectors_status_result = loop.run_until_complete(invoke_urls(connectors_status))
    from pprint import pprint as pp
    import json
    output_object = generate_metrics_object(connector_status_dict=connectors_status_result)
    # pp(output_object)
    import JMXScraper
    from urllib.parse import urlparse
    url_details = urlparse(CONNECT_REST_ENDPOINT)
    server_host_name = str(url_details.hostname + ":" + str(url_details.port))
    formmatted_jmx_data = JMXScraper.internal_get_structured_json_from_response(generate_jmx_metrics_object(output_object),
                                                                                "localhost:8083",
                                                                                server_ID="KafkaConnect")
