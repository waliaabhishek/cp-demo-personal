    ...
    for call_type in calling_object_method:
        for data_node in jmx_data_node.values():
            if "elastic" in call_type:
                task_list.append(loop.run_in_executor(_executor, ElasticSearchAppender.call_elastic_bulk, data_node))
            if "kafka" in call_type:
                task_list.append(loop.run_in_executor(_executor, KafkaAppender.produce_messages_to_kafka, data_node, JMXScraper.last_fetch_timestamp, KafkaAppender.DEFAULT_KAFKA_PRODUCER))
...
if __name__ == "__main__":
    JMXScraper.setup_everything(url_list, default_JMX_URLs,
                                poll_wait=(30 if POLL_WAIT_IN_SECS < 30 else POLL_WAIT_IN_SECS),
    ...
    if runCode:
        ...
        while (True):
            ...
            if (JMXScraper.get_metrics(force_metric_collection=False)):
                ...
                start_time = time.perf_counter()
                loop.run_until_complete(main_loop(ingestion_modules, JMXScraper.jmx_metrics_data))
                end_time = time.perf_counter() - start_time
                print(f"Ingestion comnplete cycle finished in {end_time:0.2f} seconds.")
